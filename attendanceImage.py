...
def startImage(directoryPath):
    ...
    print('Encoding Images from Database...')
    ...
    for image in imageCheckList:
        ...
        for encoding, faceFrame in zip(curEncoding, curFaceFrame):
            ...
            if faceDist[Index] < 0.55 and result:
                ...
                cv2.putText(image, f'{name}', (x1, y1 - 6), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 5)
                markAttendance(name)
                print(name + " Attended")
            else:
                cv2.rectangle(image, (x1, y1), (x2, y2), (0, 0, 255), 3)
                cv2.putText(image, f'Unknown', (x1, y1 - 6), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 5)
